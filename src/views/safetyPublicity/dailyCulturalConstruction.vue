      ...
      >
        ...
        >
          ...
          <el-form-item label="宣传方式" prop="mode">
            <sun-select v-model="dailyCulturalConstructionForm.mode" :module="'安全宣传分类'" />
          </el-form-item>
          <el-form-item>
  ...
  methods: {
    ...
    fetchData() {
      dailyCultureList(this.params).then(res => {
        ...
        const list = zhClassify(this.page.list, [['安全宣传分类', 'mode']])
    ...
    history(scope) {
      historyDailyCulture(scope.row.id).then((res) => {
        ...
        zhClassify(this.historyTable, [['安全宣传分类', 'mode']])
