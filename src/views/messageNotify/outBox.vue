  ...
  <div class="app-container">
    <div class="panel">
      ...
      >
        ...
        <template v-slot:receiverlist="scope">
          <span v-for="item in scope.row.receiverlist" :key="item.id">
            <template v-if="item.hasread===1">
              <i class="el-icon-circle-check hasread" /> {{ item.username }}
            </template>
            <template v-else>
              <i class="el-icon-circle-close unread" /> {{ item.username }}
            </template>
        ...
        <template v-slot:newreplynum="scope">

          <el-tag class="newreplynum" :type="scope.row.newreplynum ? 'danger': 'success'">{{ scope.row.newreplynum }}</el-tag>
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        {
          prop: 'receiverlist', // <String>  对应属性名
          label: '收件人', // <String>   表头标签
          align: 'left',
          slot: true
        },
        {
...

        {
          ...
          label: '发送时间', // <String>   表头标签
          slot: true
        },
        {
          prop: 'newreplynum', // <String>  对应属性名
          label: '新回复数', // <String>   表头标
          align: 'right',
          slot: true
        }
      ],
