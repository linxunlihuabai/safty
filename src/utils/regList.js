...
export function isNumber(rule, value, callback) {
  const r = /^[0-9]+(.[0-9]{1,2})?$/ // 正整数
  if (value === '') {
    return callback(new Error('经济损失不能为空'))
  } else if (!r.test(value)) {
    return callback(new Error('经济损失格式错误'))
  } else {
    return callback()
  }
}
...
export function validateIsPhone(str) {
  const reg = /^1[3|5|6|7|8|9][0-9]\d{8}$/
  return reg.test(str)
}
...
// 验证是否含有非法字符
export function validateSpecialCharator(newName, rule, value, callback) {
  const regEn = /[`!@#$%^&*()_+<>?:"{},.\/;'[\]]/im
  const regCn = /[·！#￥（——）：；“”‘、，|《。》？、【】[\]]/im
  if (regEn.test(newName) || regCn.test(newName)) {
    callback(new Error('不能有非法字符'))
  }
  callback()
}
