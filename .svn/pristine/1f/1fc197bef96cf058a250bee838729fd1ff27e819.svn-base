              ...
              <template v-if="props.row.dangerQuantity>0" slot-scope="props">
                ...
                >
                  ...
                  >
                    ...
                    <el-table-column prop="zh-level" label="隐患等级" />
                      ...
                      <template slot-scope="scope">
                        <el-button-group class="operate">
                          <el-button
                            ...
                            title="查看"
                            icon="el-icon-search"
                            @click="filePreview(scope.row.rectificationPlanFiles[0])"
                          />
                        </el-button-group>
                      </template>
                    </el-table-column>
                ...
                >
                  ...
                  >
                    ...
                    <el-table-column label="照片" :width="GLOBAL.TABLE_CELL_WIDTH.MINI">
                      <template slot-scope="scope">
                        <el-button-group class="operate">
                          <el-button type="text" size="mini" icon="el-icon-search" @click="viewPics(scope.row.acceptPhotoFiles)" />
                        </el-button-group>
                      </template>
                    </el-table-column>
                  </el-table-column>
    ...
    >
      ...
      >
        <table border="1" class="formTable">
          ...
          <tr>
            <td>
              <el-form-item label="验收照片" prop="acceptPhotoFiles">
                <sun-upload
                  ref="UPLOAD"
                  :data="GLOBAL.FILE_TYPE.OTHER"
                  :file-list.sync="acceptanceForm.acceptPhotoFiles"
                  :btn-loading.sync="btnLoading"
                />
              </el-form-item>
            </td>
...
export default {
  ...
  data() {
    return {
      ...
      acceptanceFormRules: {
        ...
        acceptPhotoFiles: [
          { required: true, message: '请上传验收照片', trigger: 'change' }
        ]
      },
  ...
  methods: {
    ...
    fetchHiddenDangerList(id, hiddenDangerCpage, hiddenDangerPageSize) {
      ...
      getHiddenDangerList(params)
        .then(res => {
          ...
          const list = pageObj.list
          zhClassify(list, [['排查领域', 'realm'], ['隐患等级', 'level']])
    ...
    expandChange(row, expandedRows) {
      ...
      if (expandedRows.length) {
        ...
        if (row) {
          ...
          this.hiddenDangerCpage = 1
          this.hiddenDangerPageSize = 5
          this.checkId = row.id
    ...
    handleHiddenDangerSizeChange() {
      this.fetchHiddenDangerList(this.checkId)
    },
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          if (state === 1) {
            ...
            // 获取文件id
            params.acceptPhotos = params.acceptPhotoFiles.map(item => {
              return item.fileId
            })
            if (params.result === 1) {
    ...
    handleRowClick(row) {
      // this.expandChange(row, this.expandedRows)
    }
  }
}
</script>
