  ...
  <div class="app-container">
    <div class="panel">
      ...
      <el-row>
        <el-col>
          ...
          <el-table ref="table" border :data="tableData" size="small" stripe>
            ...
            <el-table-column prop="major" label="主要风险" />
            <el-table-column prop="mainDanger" label="主要风险描述" />
      ...
      >
        ...
        >
          ...
          <el-form-item label="资质名称" prop="projectName">
          ...
          <el-form-item label="资质扫描上传" prop="files">
            <el-upload
              ref="upload"
              name="file"
    ...
    <el-dialog title="历史记录" :visible.sync="historyDialog" :close-on-click-modal="false" width="72%">
      <el-table ref="table" border :data="historyTable" size="small" stripe>
        <!-- 历史记录详情展示 -->
        <el-table-column prop="projectName" label="资质名称" />
        ...
        <el-table-column label="资质照片">
          <template slot-scope="scope">
            <el-button-group>
              <el-button type="primary" size="mini" icon="el-icon-search" @click="viewPics(scope.row.files)">查看资质照片</el-button>
            </el-button-group>
          </template>
        </el-table-column>
...
import { getriskPointList } from '@/api/basedata/riskPoint'
export default {
  data() {
    return {
      handle: '', // 处理方式
      ...
      formRules: {
        ...
        mainDanger: [
          { required: true, message: '请输入主要风险描述', trigger: 'blur' }
        ],
        files: [
  ...
  computed: {
    ...
    tableData() {
      ...
      return this.fuzzySearch(this.table, search, 'projectName')
    },
    queryList() {
